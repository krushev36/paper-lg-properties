...
int main(int argc, char *argv[])
{
  ...
  double *StellarMass, *diskGasMass1, *diskGasMass2, *gasFractionInRange, *gasFraction;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for(i=0; i<nb_lines; i++ )
    fscanf(file_input,"%s  %s  %lf  %s  %lf   %s  %lf  %s"
	   , char_dumb,  char_dumb, &StellarMass[i], char_dumb, &diskGasMass1[i], char_dumb, &diskGasMass2[i], char_dumb
	   );        
...
////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  fprintf(file_output, "#1.stellarMass   #2.quartil1   #3.median   #4.quartil3   #5.mean  #6.sigma2\n");
  ...
  for(mass=9.0; mass<11.0; mass+=dmass){
      ...
      j = 0;
      for(i=0; i<nb_lines; i++  ){
  	gasFraction[i] = pow(10.0, diskGasMass1[i]) + pow(10.0, diskGasMass2[i]);
  	gasFraction[i] = gasFraction[i]/(pow(10.0, diskGasMass1[i]) + pow(10.0, diskGasMass2[i]) + pow(10.0, StellarMass[i]));

  	//printf("%lf   %lf\n",gasFraction[i],StellarMass[i]);

  	if( (StellarMass[i] >= mass) && (StellarMass[i] < mass+dmass) && (gasFraction[i]>0.05) )
  	  j++;
      }
      nb_counts = j;
