...
int TreeLevelRead3(char *name_file_imput, void **params)
{
  ...
  while(!feof(file_imput_read))
      ...
      while(counter_halo<snapshot_halos)
	{
	  ...
	  halo_test.AngMoment = (double *)malloc((size_t)3*sizeof(double));
	  ...
	  dumb = fscanf(file_imput_read,"%lf %lf %lf",&halo_test.AngMoment[0], &halo_test.AngMoment[1], &halo_test.AngMoment[2]);
...
MergerTree  **TreeDataLoad3(char *name_file_imput, int tree_levels, void **params)
{
  int i, j, k, l;
  int dumb;
  int tree_id, tree_levels_max;
  ...
  while(i<tree_levels)
    {
	  ...
	  halo[i][j].AngMoment = (double *)malloc((size_t)3*sizeof(double));
	  ...
	  dumb = fscanf(file_imput_read,"%lf %lf %lf",&halo[i][j].AngMoment[0], &halo[i][j].AngMoment[1], &halo[i][j].AngMoment[2]);
  ...
  while(i<tree_levels)
    {
	  ...
	  dumb = fscanf(file_imput_read,"%lf %lf %lf",&halo[i][j].AngMoment[0], &halo[i][j].AngMoment[1], &halo[i][j].AngMoment[2]);
	  fprintf(stdout,"%.5lf %.5lf %.5lf\n", halo[i][j].AngMoment[0], halo[i][j].AngMoment[1], halo[i][j].velocity[2]);
  ...
  while(i<tree_levels)
    {
	  ...
	  dumb = fscanf(file_imput_read,"%lf %lf %lf",&halo[i][j].AngMoment[0], &halo[i][j].AngMoment[1], &halo[i][j].AngMoment[2]);
	  fprintf(file_output_write,"%11.4lf    %11.4lf    %11.4lf\n", halo[i][j].AngMoment[0], halo[i][j].AngMoment[1], halo[i][j].AngMoment[2]);
